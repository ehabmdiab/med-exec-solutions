    ...
    <footer className="bg-primary text-primary-foreground">
      <div className="container-wide py-16 grid gap-12 md:grid-cols-2 lg:grid-cols-4">
        <div className="space-y-4">
          <Logo inverted />
          <p className="text-sm text-white/70 max-w-xs">{t.footer.tagline}</p>
        </div>

        <div>
          <h4 className="text-sm font-semibold uppercase tracking-[0.18em] text-white/60 mb-4">
            {t.footer.quickLinks}
          </h4>
            ...
            {offices.map((o) => (
              <div key={o.label} className="space-y-2 text-sm">
                ...
                <p className="flex items-center gap-2 text-white/75">
                  ...
                  <a href={`tel:${o.phone.replace(/\s/g, "")}`} className="hover:text-white">{o.phone}</a>
                </p>
                <p className="flex items-center gap-2 text-white/75">
      ...
      <div className="border-t border-white/10">
        <div className="container-wide py-6 flex flex-col sm:flex-row items-center justify-between gap-2 text-xs text-white/60">
