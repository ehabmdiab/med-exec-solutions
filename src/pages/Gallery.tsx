...
function GalleryTile({
  ...
  const handlePlay = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      console.error("Inline video play failed", err);
    }
  };
  ...
  if (item.type === "image") {
    return (
      <button
        ...
        onClick={onExpand}
        onContextMenu={noContext}
        className="absolute inset-0 w-full h-full text-start cursor-zoom-in"
        aria-label={item.title}
      >
        <img
          src={item.src}
        ...
        />
        <div className="absolute inset-0 z-10" onContextMenu={noContext} />
        <TileOverlay item={item} locale={locale} hideGradient={false} />
      </button>
    );
  }
  ...
  // Video tile
  return (
    <div className="absolute inset-0 w-full h-full" onContextMenu={noContext}>
      ...
      <video
        ...
        onContextMenu={noContext}
        className={`absolute inset-0 w-full h-full bg-black ${playing ? "object-contain z-20" : "object-cover opacity-0 pointer-events-none"}`}
      />
      ...
      {/* Overlay info (only when not playing) */}
      {!playing && <TileOverlay item={item} locale={locale} hideGradient={false} />}

      {/* Expand-to-lightbox corner button (always available) */}
      <button
        type="button"
        onClick={(e) => { e.stopPropagation(); onExpand(); }}
        aria-label={locale === "ar" ? "تكبير" : "Expand"}
        className="absolute top-3 end-3 z-40 flex h-9 w-9 items-center justify-center rounded-full bg-black/50 hover:bg-black/70 backdrop-blur border border-white/30 text-white transition"
      >
...
function TileOverlay({ item, locale, hideGradient }: { item: MediaItem; locale: string; hideGradient: boolean }) {
  return (
    <>
      {!hideGradient && (
        <div className="absolute inset-0 z-20 bg-gradient-to-t from-primary/95 via-primary/30 to-transparent opacity-90 group-hover:opacity-100 transition-opacity pointer-events-none" />
      )}
      <div className="absolute inset-x-0 bottom-0 z-30 p-4 md:p-5 text-white translate-y-2 group-hover:translate-y-0 transition-transform duration-500 pointer-events-none">
        <div className="flex items-center gap-2 text-[10px] font-semibold tracking-[0.18em] uppercase text-accent-gold">
          <span className="h-px w-6 bg-accent-gold" />
          {item.type === "video" ? (locale === "ar" ? "فيديو" : "Video") : (locale === "ar" ? "صورة" : "Photo")}
        </div>
      ...
      <span className="absolute top-3 start-3 z-30 text-[10px] font-bold tracking-widest text-white/70 uppercase opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
        AUH ©
      </span>
    </>
  );
